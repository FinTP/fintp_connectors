...
void sigTrapHandler( int signo )
{
	...
	//sprintf( corefilename, "/qpay/core", progname.c_str(), crttime.c_str() );
	...
	if ( argc > 1 )
	{
		if( !strcmp( argv[ 1 ], "--version" ) )
		{
			cout << "Version info : " << 
				...
				endl << FinTPConnector::VersionInfo::Url() <<
				endl << "\t" << FinTP::VersionInfo::Name() << "\t" << FinTP::VersionInfo::Id() <<
				endl << FinTP::VersionInfo::Url() <<
#ifdef USING_REGULATIONS
				endl << "\t" << qPayRR::VersionInfo::Name() << "\t" << qPayRR::VersionInfo::Id() <<
				endl << qPayRR::VersionInfo::Url() <<
#endif //USING_REGULATIONS
				endl << endl;
